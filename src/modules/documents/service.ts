...
} from "./types/template-kind";
import {
  INVOICE_NUMBER_PLACEHOLDER,
  PACKING_SLIP_NUMBER_PLACEHOLDER,
} from "./types/constants";
import {
  generateInvoice,
  validateInputForProvidedKind,
} from "./services/generators/invoice-generator";
import {
  generatePackingSlip,
...
class DocumentsModuleService extends MedusaService({
  ...
  private async getInvoiceForcedNumber(): Promise<string | undefined> {
    ...
    if (
      ...
      lastDocumentInvoiceSettings.length &&
      lastDocumentInvoiceSettings[0].forcedNumber
    ) {
      const nextNumber: string =
        lastDocumentInvoiceSettings[0].forcedNumber.toString();
      return nextNumber;
    }
    return undefined;
  }
  ...
  private async getNextInvoiceNumber(resetForcedNumber?: boolean) {
    const forcedNumber: string | undefined =
      await this.getInvoiceForcedNumber();
  ...
  async getPackingSlip(
    ...
    if (includeBuffer) {
      const packingSlip = await this.retrieveDocumentPackingSlip(
        packingSlipId,
        {
      ...
      if (packingSlip) {
        const calculatedTemplateKind = this.calculatePackingSlipTemplateKind(
          packingSlip.packingSlipSettings,
        );
        const buffer = await generatePackingSlip(
          calculatedTemplateKind,
          packingSlip.settings,
          packingSlip,
          order,
        );
        return {
  ...
  async generateTestPackingSlip(
    ...
    console.log("lastDocumentSettings", lastDocumentSettings);

    if (lastDocumentSettings && lastDocumentSettings.length) {
      const nextNumber: string = await this.getNextPackingSlipNumber();

      const [validationPassed, info] = validatePackingSlipInputForProvidedKind(
        templateKind,
        ...
        if (validationPassed) {
          const testInvoice: DocumentInvoiceDTO = {
            ...
            displayNumber: invoiceSettings.numberFormat
              ? invoiceSettings.numberFormat.replace(
                  INVOICE_NUMBER_PLACEHOLDER,
                  nextNumber,
                )
              : nextNumber,
          ...
          const buffer = await generateInvoice(
            templateKind,
            { ...lastDocumentSettings[0], ...storeDetail },
            testInvoice,
            order,
            storeDetail,
          );
    ...
    if (order) {
      ...
      if (lastDocumentSettings && lastDocumentSettings.length) {
        const lastDocumentInvoiceSettings =
          await this.listDocumentInvoiceSettings(
            {},
            {
              order: {
                created_at: "DESC",
        ...
        if (lastDocumentInvoiceSettings && lastDocumentInvoiceSettings.length) {
          ...
          if (validationPassed) {
            ...
            const nextNumber: string =
              await this.getNextInvoiceNumber(RESET_FORCED_NUMBER);
            ...
            const invoiceResult =
              await this.createDocumentInvoices(entryInvoice);
  ...
  private calculatePackingSlipTemplateKind(
    documentPackingSlipSettings: any,
  ): PackingSlipTemplateKind {
  ...
  async generatePackingSlipForOrder(order: OrderDTO): Promise<any> {
    ...
    if (lastDocumentSettings && lastDocumentSettings.length) {
      const lastDocumentPackingSlipSettings =
        await this.listDocumentPackingSlipSettings(
          {},
          {
            order: {
              created_at: "DESC",
      ...
      if (
        lastDocumentPackingSlipSettings &&
        ...
        const [validationPassed, info] =
          validatePackingSlipInputForProvidedKind(
            calculatedTemplateKind,
            lastDocumentSettings[0],
          );
        ...
        if (validationPassed) {
          ...
          const entryPackingSlip: any = {
            ...
            displayNumber: packingSlipSettings.numberFormat
              ? packingSlipSettings.numberFormat.replace(
                  PACKING_SLIP_NUMBER_PLACEHOLDER,
                  nextNumber,
                )
              : nextNumber,
          ...
          const packingSlipResult =
            await this.createDocumentPackingSlips(entryPackingSlip);
  ...
  async updateInvoiceTemplate(
    ...
    if (lastDocumentInvoiceSettings && lastDocumentInvoiceSettings.length) {
      ...
      const result =
        await this.createDocumentInvoiceSettings(newDocumentSettings);
      return result;
    } else {
      const result = await this.createDocumentInvoiceSettings({
  ...
  async updatePackingSlipTemplate(
    ...
    if (
      ...
      const newDocumentSettings = {
        template:
          packingSlipTemplate ?? lastDocumentPackingSlipSettings[0].template,
      };
      const result =
        await this.createDocumentPackingSlipSettings(newDocumentSettings);
      return result;
    } else {
  ...
  async updatePackingSlipSettings(
    ...
    if (
      ...
      const result = await this.createDocumentPackingSlipSettings({
        numberFormat:
          newFormatNumber ?? lastDocumentPackingSlipSettings[0].numberFormat,
  ...
  async updateInvoiceSettings(
    ...
    if (lastDocumentInvoiceSettings && lastDocumentInvoiceSettings.length) {
      const result = await this.createDocumentInvoiceSettings({
        numberFormat:
          newFormatNumber ?? lastDocumentInvoiceSettings[0].numberFormat,
        forcedNumber: forcedNumber
          ? parseInt(forcedNumber)
          : lastDocumentInvoiceSettings[0].forcedNumber,
        template: invoiceTemplate ?? lastDocumentInvoiceSettings[0].template,
