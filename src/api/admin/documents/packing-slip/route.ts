...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    if (order) {
      const result =
        await documentsModuleService.generatePackingSlipForOrder(order);
      if (result.packingSlip) {
        ...
        await assignPackingSlipToOrderWorkflow(req.scope).run({
          input: {
            orderId: order.id,
...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    if (orderWithPackingSlip.document_packing_slip && orderId) {
      ...
      const result = await documentsModuleService.getPackingSlip(
        orderDto,
        orderWithPackingSlip.document_packing_slip.id,
